...
struct APIClient: Sendable {
    ...
    func uploadMedia(
        ...
        variant: String = "compressed",
        audioGroupCount: Int? = nil
    ...
    func downloadCheckInMediaFile(mediaId: String, variant: String = "compressed") async throws -> URL {
        var components = URLComponents(url: endpoint("api/v1/checkin-media/\(mediaId)"), resolvingAgainstBaseURL: false)
        components?.queryItems = [
            URLQueryItem(name: "variant", value: variant)
        ]
    ...
    func review(reviewId: String) async throws -> ReviewPayload {
    ...
    func sendReviewFeedback(reviewId: String, type: String, enabled: Bool, note: String? = nil) async throws -> ReviewPayload {
        var request = try authorizedRequest(url: endpoint("api/v1/reviews/\(reviewId)/feedback"))
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.httpBody = try encoder.encode(ReviewFeedbackRequest(type: type, enabled: enabled, note: note))
        let response: ReviewFeedbackResponse = try await send(request)
        return response.review
    }
    ...
    private func multipartBodyFile(
        ...
        do {
            ...
            if media.isAudio, let audioGroupCount {
                try writeMultipartField("audioGroupCount", value: "\(audioGroupCount)", boundary: boundary, to: handle)
            }
    ...
    private func uploadFile(for media: CheckInMedia, variant: String) throws -> (url: URL, filename: String, mimeType: String) {
        guard variant == "compressed",
              (media.isImage || media.isAudio),
              !media.localCompressedPath.isEmpty else {
            throw APIError.missingUploadFile
        }
        ...
        return (
            URL(fileURLWithPath: media.localCompressedPath),
            "\(media.id).\(ext)",
            media.mimeType ?? (media.isAudio ? "audio/mp4" : "image/jpeg")
        )
    }
    ...
    private func uploadTimeout(for media: TimelineMedia, bodyBytes: Int64) -> TimeInterval {
        ...
        if media.isDocument {
            return min(300, max(120, 60 + megabytes * 20))
        }
...
enum APIError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .httpStatus(let status, let body):
            return "HTTP \(status): \(body.sanitizedHTTPErrorBody(status: status))"
        }
    }
}
...
private extension String {
    ...
    func sanitizedHTTPErrorBody(status: Int) -> String {
        let trimmed = trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else {
            return "Server returned an empty error response."
        }

        if trimmed.looksLikeHTMLDocument || trimmed.lowercased().contains("error code: 502") {
            if status == 502 {
                return "Bad gateway from server endpoint. Check the configured URL or tunnel."
            }
            return "Server endpoint returned an HTML error page."
        }

        if let message = trimmed.jsonErrorMessage {
            return message
        }
    ...
    var looksLikeHTMLDocument: Bool {
        let normalized = lowercased()
