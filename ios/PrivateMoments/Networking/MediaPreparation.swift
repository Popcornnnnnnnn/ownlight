...
enum DocumentMediaImporter {
    static func preparePDF(from sourceURL: URL) throws -> PreparedMomentMedia {
        guard FileManager.default.fileExists(atPath: sourceURL.path) else {
            throw MediaPreparationError.documentFileUnavailable
        }

        guard Self.isPDF(sourceURL) else {
            throw MediaPreparationError.unsupportedDocument
        }

        let directory = try AppDirectories.draftMediaDirectory()
        let mediaId = UUID().uuidString
        let destinationURL = directory.appending(path: "composer-document-\(mediaId).pdf")
        if FileManager.default.fileExists(atPath: destinationURL.path) {
            try FileManager.default.removeItem(at: destinationURL)
        }
        try FileManager.default.copyItem(at: sourceURL, to: destinationURL)
