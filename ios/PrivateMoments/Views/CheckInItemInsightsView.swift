    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            ZStack(alignment: .topLeading) {
                ...
                .background(Color.secondary.opacity(0.035), in: RoundedRectangle(cornerRadius: 8, style: .continuous))
    ...
    private func drawSeries(context: GraphicsContext, layout: CheckInTimeLineChartLayout) {
        ...
        for plotPoint in layout.plotPoints {
            ...
            if let previous {
                ...
                context.stroke(segment, with: .color(color), style: StrokeStyle(lineWidth: 2.6, lineCap: .round))
            }

            if isSelected {
                context.fill(
                    Path(ellipseIn: CGRect(x: plotPoint.position.x - 7, y: plotPoint.position.y - 7, width: 14, height: 14)),
                    with: .color(color.opacity(0.20))
                )
                context.fill(
                    Path(ellipseIn: CGRect(x: plotPoint.position.x - 5, y: plotPoint.position.y - 5, width: 10, height: 10)),
                    with: .color(color)
                )
                context.stroke(
                    Path(ellipseIn: CGRect(x: plotPoint.position.x - 5, y: plotPoint.position.y - 5, width: 10, height: 10)),
...
struct CheckInInsightHint: View {
    let title: String
