        ...
        VStack(alignment: .leading, spacing: 14) {
            if insight.hasData {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    CheckInHeatmapLegend(color: color, language: language)
                }
                ...
                Button {
                    ...
                    VStack(spacing: 3) {
                        RoundedRectangle(cornerRadius: 3, style: .continuous)
                            ...
                            .scaleEffect(isSelected(bucket) ? 1.05 : 1)
                            .animation(.snappy(duration: 0.14), value: isSelected(bucket))
            ...
            HStack(spacing: 3) {
                ForEach(0..<24, id: \.self) { hour in
                    ...
                    RoundedRectangle(cornerRadius: 2, style: .continuous)
                        ...
                        .scaleEffect(isSelected(hour: hour) ? 1.18 : 1)
                        .animation(.snappy(duration: 0.14), value: isSelected(hour: hour))
                        .contentShape(Rectangle())
...
private struct CheckInHeatmapLegend: View {
    let color: Color
    let language: AppResolvedLanguage

    var body: some View {
        HStack(spacing: 7) {
            Text(L10n.t("Less", language))
            HStack(spacing: 3) {
                ForEach([0.18, 0.34, 0.54, 0.78], id: \.self) { opacity in
                    RoundedRectangle(cornerRadius: 2, style: .continuous)
                        .fill(color.opacity(opacity))
                        .frame(width: 18, height: 8)
                }
            }
            Text(L10n.t("More", language))
            Spacer(minLength: 0)
            CheckInInsightHint(title: L10n.t("Tap or drag to inspect", language))
        }
...
private func insightOpacity(for intensity: Double, isEmpty: Bool) -> Double {
    ...
    return 0.24 + min(max(intensity, 0), 1) * 0.66
}
