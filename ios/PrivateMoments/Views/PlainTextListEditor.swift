...
private let markdownTextEditorLogger = Logger(
    subsystem: Bundle.main.bundleIdentifier ?? "PrivateMoments",
    category: "MarkdownTextEditor"
)

struct MarkdownTextEditor: UIViewRepresentable {
    @Binding var text: String
    var onPasteImages: (([Data]) -> Void)? = nil
    var onUserTextChange: ((String) -> Void)? = nil
    var externalResetToken: Int = 0
    ...
    func updateUIView(_ uiView: UITextView, context: Context) {
        ...
        context.coordinator.onUserTextChange = onUserTextChange
        context.coordinator.textView = uiView
        context.coordinator.autoFocus = autoFocus
        configureImagePasteHandler(for: uiView, context: context)
        uiView.isScrollEnabled = false
        context.coordinator.keepEmptyEditorCaretAtTop(uiView)
        context.coordinator.applyAutoFocusIfNeeded()
        let allowsExternalReset = context.coordinator.consumeExternalResetIfNeeded(externalResetToken)

        let currentText = uiView.text ?? ""
        if currentText != text,
           !context.coordinator.hasMarkedText(in: uiView) {
            if context.coordinator.shouldPreserveTextViewText(
                currentText: currentText,
                incomingText: text,
                isFirstResponder: uiView.isFirstResponder,
                allowsExternalReset: allowsExternalReset
            ) {
                context.coordinator.restoreBinding(to: currentText)
                return
            }

            uiView.text = text
            context.coordinator.applyMarkdownStyling(to: uiView, force: true)
            if allowsExternalReset {
                context.coordinator.placeCaretAtBeginning(uiView)
            }
            context.coordinator.keepEmptyEditorCaretAtTop(uiView)
    ...
    final class Coordinator: NSObject, UITextViewDelegate {
        ...
        var onUserTextChange: ((String) -> Void)?
        var autoFocus: Bool
        weak var textView: UITextView?

        private var isApplyingMarkdownStyling = false
        private var lastExternalResetToken: Int
        ...
        init(
            text: Binding<String>,
            onPasteImages: (([Data]) -> Void)?,
            onUserTextChange: ((String) -> Void)?,
            externalResetToken: Int,
            autoFocus: Bool
        ) {
            self.text = text
            ...
            self.lastExternalResetToken = externalResetToken
        }
        ...
        func shouldPreserveTextViewText(
            currentText: String,
            incomingText: String,
            isFirstResponder: Bool,
            allowsExternalReset: Bool
        ) -> Bool {
            guard incomingText.isEmpty,
                  !allowsExternalReset,
                  !currentText.isEmpty else {
                return false
            }

            markdownTextEditorLogger.warning(
                "markdown_editor skipped external empty replacement currentLength=\(currentText.count, privacy: .public) firstResponder=\(isFirstResponder, privacy: .public)"
            )
            return true
        }

        func consumeExternalResetIfNeeded(_ externalResetToken: Int) -> Bool {
            guard externalResetToken != lastExternalResetToken else {
                return false
            }

            lastExternalResetToken = externalResetToken
            return true
        }

        func restoreBinding(to preservedText: String) {
            let binding = text
            DispatchQueue.main.async {
                ...
                binding.wrappedValue = preservedText
            }
        }

        func keepEmptyEditorCaretAtTop(_ textView: UITextView) {
            guard (textView.text ?? "").isEmpty else {
                return
            }

            placeCaretAtBeginning(textView)
        }

        func placeCaretAtBeginning(_ textView: UITextView) {
        ...
        func applyAutoFocusIfNeeded() {
            guard autoFocus, !didApplyAutoFocus, let textView else {
                return
            }

            didApplyAutoFocus = true
            DispatchQueue.main.async { [weak textView] in
                guard let textView, textView.window != nil, !textView.isFirstResponder else {
                    return
                }
...
struct PlainTextListEditor: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: UITextView, context: Context) {
        ...
        context.coordinator.dismissRequest = dismissRequest
        context.coordinator.textView = uiView
        configureImagePasteHandler(for: uiView, context: context)
        ...
        context.coordinator.syncFirstResponder()
    }
    ...
    final class Coordinator: NSObject, UITextViewDelegate {
        ...
        private var lastHandledDismissRequest: Int?
        ...
        func syncFirstResponder() {
            ...
            if lastHandledDismissRequest != dismissRequest {
                lastHandledDismissRequest = dismissRequest
            ...
            guard lastHandledFocusRequest != focusRequest else {
                return
            }
