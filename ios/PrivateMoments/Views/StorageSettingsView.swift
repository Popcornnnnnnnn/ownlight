...
struct StorageSummaryLink: View {
    ...
    var body: some View {
        Group {
            ...
            NavigationLink {
                ...
                summaryLinkLabel(title: "Diagnostics", detail: diagnosticsSummary, badges: diagnosticsBadges)
            }
        }
        .task {
            await refresh()
    ...
    private var diagnosticsSummary: String {
        ...
        var parts = [localDiagnosticsText(for: localStats)]
    ...
    private var diagnosticsBadges: [SettingsStatusBadgeModel] {
        guard let localStats else {
            return [
                SettingsStatusBadgeModel(
                    title: L10n.t("Checking", appLanguage),
                    systemImage: "clock",
                    tone: .neutral
                )
            ]
        ...
        var badges = [localDiagnosticsBadge(for: localStats)]

        if localStats.failedUploads > 0 {
            badges.append(
                SettingsStatusBadgeModel(
                    title: "\(localStats.failedUploads) \(L10n.t("failed uploads", appLanguage))",
                    systemImage: "exclamationmark",
                    tone: .danger
                )
            )
        } else if localStats.missingMediaDownloads > 0 {
            badges.append(
                SettingsStatusBadgeModel(
                    title: "\(localStats.missingMediaDownloads) \(L10n.t("Missing media", appLanguage))",
    ...
    private func summaryLinkLabel(title: String, detail: String, badges: [SettingsStatusBadgeModel] = []) -> some View {
    ...
    private func localDiagnosticsBadge(for stats: LocalStorageStats) -> SettingsStatusBadgeModel {
        if stats.pendingChanges == 0 && stats.pendingUploads == 0 && stats.failedUploads == 0 && stats.missingMediaDownloads == 0 {
            return SettingsStatusBadgeModel(
                title: L10n.t("Local-only", appLanguage),
                systemImage: "iphone",
                tone: .neutral
            )
        }
        ...
        if stats.failedUploads > 0 {
            return SettingsStatusBadgeModel(
                title: "\(stats.failedUploads) \(L10n.t("failed uploads", appLanguage))",
                systemImage: "exclamationmark",
                tone: .danger
            )
        }
        ...
        if stats.pendingUploads > 0 {
            return SettingsStatusBadgeModel(
                title: L10n.t("Local work queued", appLanguage),
                systemImage: "clock",
                tone: .warning
            )
        }
    ...
    private func localDiagnosticsText(for stats: LocalStorageStats) -> String {
        if stats.pendingChanges == 0 && stats.pendingUploads == 0 && stats.failedUploads == 0 && stats.missingMediaDownloads == 0 {
            return L10n.t("Local diagnostics", appLanguage)
        }
        ...
        if stats.pendingUploads > 0 {
            return L10n.t("Local work queued", appLanguage)
        }
...
struct StorageDetailsView: View {
    ...
    private func storageSummarySection(_ stats: LocalStorageStats, serverStatus: AdminStatusResponse?) -> some View {
        Section(L10n.t("Summary", appLanguage)) {
            ...
            } else if replicationDiagnosticsAvailable(serverStatus: serverStatus) {
    ...
    private func diagnosticsSummarySection(_ stats: LocalStorageStats, serverStatus: AdminStatusResponse?) -> some View {
        Section(L10n.t("Summary", appLanguage)) {
            LabeledContent(L10n.t("This iPhone", appLanguage), value: localDiagnosticsText(for: stats))

            if replicationDiagnosticsAvailable(serverStatus: serverStatus) {
    ...
    private func diagnosticsMenuSection(
        ...
        if replicationDiagnosticsAvailable(serverStatus: serverStatus) {
            Section(L10n.t("Advanced", appLanguage)) {
                NavigationLink {
    ...
    private func replicationDiagnosticsAvailable(serverStatus: AdminStatusResponse?) -> Bool {
        store.isAuthenticated || serverStatus != nil
    }
    ...
    private func syncDoctorSection(_ stats: LocalStorageStats, serverStatus: AdminStatusResponse?) -> some View {
        ...
        Section(L10n.t("Recommended", appLanguage)) {
            ...
            if let action = diagnosis.recommendedAction {
                VStack(alignment: .leading, spacing: 4) {
                    Text(L10n.t("Next safe step", appLanguage))
                        .font(.footnote.weight(.semibold))
                    Text(L10n.t(diagnosis.recommendedGuidanceKey, appLanguage))
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                }
                .padding(.top, 2)
