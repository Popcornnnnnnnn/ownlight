...
enum MomentDateFormatter {
    ...
    static func commentRelativeTitle(
        ...
        if daysBetween(date, and: now, calendar: calendar) == 1 {
            return "Yesterday"
        }
    ...
    static func clockTimeTitle(
        for date: Date,
        calendar: Calendar = .current
    ) -> String {
        clockTime(for: date, calendar: calendar)
    }

    static func detailDateTitle(
        for date: Date,
        calendar: Calendar = .current,
        language: AppResolvedLanguage = .english
    ) -> String {
    ...
    static func mediumDateTimeTitle(
        for date: Date,
        calendar: Calendar = .current,
        language: AppResolvedLanguage = .english
    ) -> String {
    ...
    static func twentyFourHourLocale(for language: AppResolvedLanguage) -> Locale {
        switch language {
