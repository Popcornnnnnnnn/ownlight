...
enum CalendarDayReviewFilter: String, CaseIterable, Identifiable, Hashable {
    ...
    func includes(_ item: MomentFeedItem) -> Bool {
        ...
        case .checkIn(let checkIn):
            ...
            case .audio:
                return checkIn.media.contains { $0.isAudio }
            case .video, .favorites, .comments:
                return false
            }
        }
    }
...
struct CalendarReviewMonthStats: Equatable {
    ...
    init(days: [CalendarReviewDay]) {
        ...
        audioCount = media.filter(\.isAudio).count + checkInMedia.filter(\.isAudio).count
...
enum CalendarReviewBuilder {
    static func month(
        ...
        return CalendarReviewMonth(
            ...
            weekdays: weekdays(calendar: calendar, language: language),
            days: days
        )
    }
    ...
    private static func weekdays(calendar: Calendar, language: AppResolvedLanguage) -> [CalendarReviewWeekday] {
            ...
            .filter { hint in
                ...
                case .audio:
                    return items.contains { $0.media.contains { $0.isAudio } }
                        || checkIns.contains { $0.media.contains { $0.isAudio } }
                case .video:
