...
struct EditTagsView: View {
    ...
    private var activeAreas: [TopicTagArea] {
        TopicTagArea.displayAreas
    }

    private func activeTopics(in area: TopicTagArea) -> [TimelineTag] {
        store.activeTopicTags
    ...
    private func save() {
        ...
        isSaving = true
        let primary = item.primaryTag?.tagId
    ...
    private func addTopic() {
        ...
        Task {
            let areaId = TopicTagArea.inferredArea(forTopicName: name).rawValue
            if let tag = await store.createTag(type: "topic", name: name, areaId: areaId) {
                await MainActor.run {
...
struct DetailTagBadge: View {
    ...
    var body: some View {
        Text(L10n.tagName(tag, language: appLanguage))
            ...
            .padding(.vertical, 6)
            .background(chipColor.opacity(0.24), in: RoundedRectangle(cornerRadius: 13, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: 13, style: .continuous)
                    .stroke(chipColor.opacity(0.36), lineWidth: 0.6)
            )
    }
