...
extension LocalDatabase {
    ...
    func fetchPendingCheckInMediaReadyForUpload(limit: Int = 50) throws -> [CheckInMedia] {
        let statement = try prepare(
            ...
            SELECT m.id, m.entryId, m.kind, m.localCompressedPath, m.remoteCompressedPath,
                   m.uploadStatus, m.uploadError, m.mimeType, m.durationSeconds, m.transcriptionText,
                   m.transcriptionStatus, m.transcriptionError, m.transcriptionUpdatedAt, m.sortOrder,
                   m.checksum, m.createdAt, m.updatedAt, m.deletedAt
            FROM local_checkin_media m
            JOIN local_checkin_entries e ON e.id = m.entryId
            JOIN local_checkin_items i ON i.id = e.itemId
            WHERE m.uploadStatus IN ('pending', 'failed')
    ...
    func updateCheckInMediaTranscriptionStatus(
        mediaId: String,
        status: String,
        error: String?,
        updatedAt: Date
    ...
    func updateCheckInMediaTranscription(
        mediaId: String,
    ...
    func upsertCheckInMediaOnly(_ media: CheckInMedia) throws {
        let statement = try prepare(
            ...
            ON CONFLICT(id) DO UPDATE SET
                ...
                transcriptionText = excluded.transcriptionText,
                transcriptionStatus = excluded.transcriptionStatus,
                transcriptionError = excluded.transcriptionError,
                transcriptionUpdatedAt = excluded.transcriptionUpdatedAt,
        ...
        try bind(media.durationSeconds, to: 9, in: statement)
        try bind(media.transcriptionText, to: 10, in: statement)
        try bind(media.transcriptionStatus, to: 11, in: statement)
        try bind(media.transcriptionError, to: 12, in: statement)
        try bind(media.transcriptionUpdatedAt, to: 13, in: statement)
        try bind(media.sortOrder, to: 14, in: statement)
        try bind(media.checksum, to: 15, in: statement)
        try bind(media.createdAt, to: 16, in: statement)
        try bind(media.updatedAt, to: 17, in: statement)
        try bind(media.deletedAt, to: 18, in: statement)
        try stepDone(statement)
    }
    ...
    func checkInMedia(_ statement: OpaquePointer) throws -> CheckInMedia {
        CheckInMedia(
            ...
            durationSeconds: optionalDouble(statement, 8),
            transcriptionText: optionalText(statement, 9),
            transcriptionStatus: optionalText(statement, 10) ?? "not_requested",
            transcriptionError: optionalText(statement, 11),
            transcriptionUpdatedAt: try optionalDate(statement, 12),
            sortOrder: Int(sqlite3_column_int64(statement, 13)),
            checksum: optionalText(statement, 14),
            createdAt: try date(statement, 15),
            updatedAt: try date(statement, 16),
            deletedAt: try optionalDate(statement, 17)
        )
    }
}
