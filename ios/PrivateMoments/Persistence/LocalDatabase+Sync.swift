...
extension LocalDatabase {
    ...
    func fetchAudioMediaCountsByPost() throws -> [String: Int] {
        let statement = try prepare(
            """
            SELECT postId, COUNT(*)
            FROM local_media
        ...
        while sqlite3_step(statement) == SQLITE_ROW {
            counts[try text(statement, 0)] = Int(sqlite3_column_int64(statement, 1))
        }
    ...
    func applyCheckInAISummaryUpdated(_ summary: CheckInAISummary, serverVersion: Int) throws {
        guard try fetchCheckInEntry(id: summary.entryId) != nil else {
            throw StoreError.invalidServerChange("checkin_ai_summary_updated references a missing parent check-in entry")
        }

        guard try fetchCheckInMedia(id: summary.mediaId) != nil else {
            throw StoreError.invalidServerChange("checkin_ai_summary_updated references missing check-in media")
        }
    ...
    func applyCheckInAISummaryDeleted(_ summary: CheckInAISummary, serverVersion: Int) throws {
        guard try fetchCheckInEntry(id: summary.entryId) != nil else {
            throw StoreError.invalidServerChange("checkin_ai_summary_deleted references a missing parent check-in entry")
        }

        let deletedSummary = CheckInAISummary(
            id: summary.id,
            ...
            documentBlocks: summary.documentBlocks,
        ...
        try upsertCheckInAISummary(deletedSummary)
        _ = serverVersion
    }
