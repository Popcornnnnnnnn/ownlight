...
extension LocalDatabase {
    ...
    func fetchMemoryLinkHistory() throws -> [MemoryLinkHistoryEntry] {
        let statement = try prepare(
            """
            SELECT id, postId, shownDate, sourceWindow, score, shownAt, openedAt, dismissedAt
            FROM local_memory_link_events
            ORDER BY shownDate DESC, shownAt DESC
            """
        )
        defer {
        ...
        var entries: [MemoryLinkHistoryEntry] = []
        while sqlite3_step(statement) == SQLITE_ROW {
            guard let sourceWindow = MemoryLinkSourceWindow(rawValue: try text(statement, 3)) else {
                continue
            }
            ...
            entries.append(
                MemoryLinkHistoryEntry(
                    ...
                    shownDate: try date(statement, 2),
                    sourceWindow: sourceWindow,
                    score: Int(sqlite3_column_int64(statement, 4)),
                    shownAt: try date(statement, 5),
                    openedAt: try optionalDate(statement, 6),
                    dismissedAt: try optionalDate(statement, 7)
                )
            )
        }
    ...
    func recordMemoryLinkShown(_ link: MemoryLink, shownAt: Date = Date()) throws {
        let statement = try prepare(
            """
            INSERT INTO local_memory_link_events
                (id, postId, shownDate, sourceWindow, score, shownAt, openedAt, dismissedAt)
            VALUES (?, ?, ?, ?, ?, ?, NULL, NULL)
            ON CONFLICT(shownDate) DO UPDATE SET
                postId = excluded.postId,
                sourceWindow = excluded.sourceWindow,
                score = excluded.score,
                shownAt = local_memory_link_events.shownAt
            """
        )
        defer {
            sqlite3_finalize(statement)
        ...
        try bind("memory-link-\(Int(link.shownDate.timeIntervalSince1970))-\(link.postId)", to: 1, in: statement)
        try bind(link.postId, to: 2, in: statement)
        try bind(link.shownDate, to: 3, in: statement)
        try bind(link.sourceWindow.rawValue, to: 4, in: statement)
        try bind(link.score, to: 5, in: statement)
        try bind(shownAt, to: 6, in: statement)
        try stepDone(statement)
    }

    func markMemoryLinkOpened(postId: String, shownDate: Date, openedAt: Date = Date()) throws {
        let statement = try prepare(
            """
            UPDATE local_memory_link_events
        ...
        try bind(openedAt, to: 1, in: statement)
        try bind(postId, to: 2, in: statement)
        try bind(shownDate, to: 3, in: statement)
        try stepDone(statement)
    }
