...
extension TimelineStore {
    func apply(change: ServerChange, database: LocalDatabase) throws {
        ...
        case "checkin_ai_summary_updated":
            let summary = try parseCheckInAISummaryPayload(change.payload, changeType: "checkin_ai_summary_updated")
            try database.applyCheckInAISummaryUpdated(summary, serverVersion: change.version)

        case "checkin_ai_summary_deleted":
            let summary = try parseCheckInAISummaryPayload(change.payload, changeType: "checkin_ai_summary_deleted")
            try database.applyCheckInAISummaryDeleted(summary, serverVersion: change.version)
    ...
    private func parseAISummaryPayload(
        ...
        return TimelineAISummary(
            ...
            inputTokenCount: payload["inputTokenCount"]?.intValue,
            outputTokenCount: payload["outputTokenCount"]?.intValue,
            totalTokenCount: payload["totalTokenCount"]?.intValue,
    ...
    private func parseCheckInAISummaryPayload(
        _ payload: [String: JSONValue],
        changeType: String
    ) throws -> CheckInAISummary {
        guard let id = payload["id"]?.stringValue,
              let entryId = payload["entryId"]?.stringValue,
              let mediaId = payload["mediaId"]?.stringValue,
              let status = payload["status"]?.stringValue,
              let promptVersion = payload["promptVersion"]?.stringValue,
              let createdAtValue = payload["createdAt"]?.stringValue,
    ...
    private func parseTagPayload(
        ...
        return TimelineTag(
            ...
            areaId: payload["areaId"]?.stringValue
        )
    }
