...
extension TimelineStore {
    ...
    func localURLForDownloadedMedia(_ media: TimelineMedia, variant: String = "compressed") throws -> URL {
        ...
        return try safeDownloadedMediaURL(id: media.id, suffix: suffix, fileExtension: fileExtension)
    }
    ...
    func localURLForDownloadedCheckInMedia(_ media: CheckInMedia) throws -> URL {
        ...
        return try safeDownloadedMediaURL(
            id: media.id,
            suffix: "",
            fileExtension: remoteExtension ?? media.preferredFileExtension
        )
    }

    private func safeDownloadedMediaURL(id: String, suffix: String, fileExtension: String) throws -> URL {
        guard isSafeMediaCacheToken(id), isSafeMediaCacheExtension(fileExtension) else {
            throw StoreError.invalidServerChange("Unsafe media cache filename")
        }

        let directory = try AppDirectories.mediaDirectory().standardizedFileURL
        let url = directory.appending(path: "\(id)\(suffix).\(fileExtension)").standardizedFileURL
        guard url.path.hasPrefix(directory.path + "/") else {
            throw StoreError.invalidServerChange("Unsafe media cache path")
        }

        return url
    }

    private func isSafeMediaCacheToken(_ value: String) -> Bool {
        value.range(of: #"^[A-Za-z0-9_-]{1,128}$"#, options: .regularExpression) != nil
    }

    private func isSafeMediaCacheExtension(_ value: String) -> Bool {
        value.range(of: #"^[A-Za-z0-9]{1,10}$"#, options: .regularExpression) != nil
    }
    ...
    func persistPreparedMedia(
        ...
        if !audio.isEmpty {
            return try Self.persistPreparedAudioMedia(postId: postId, audio: audio, createdAt: createdAt)
        }
    ...
    nonisolated static func persistPreparedAudioMedia(
        postId: String,
        audio: [PreparedMomentMedia],
        createdAt: Date
    ) throws -> [TimelineMedia] {
        try audio.prefix(9).enumerated().map { index, draft in
            try Self.persistFileMedia(
                postId: postId,
                draft: draft,
                sortOrder: index,
    ...
    nonisolated static func persistFileMedia(
        ...
        let fileExtension = safeFileExtension(for: draft)
        ...
        return TimelineMedia(
            ...
            transcriptionText: nil,
            transcriptionStatus: draft.kind == "audio" || draft.kind == "video" ? "pending" : "not_applicable",
            transcriptionError: nil,
            transcriptionUpdatedAt: nil,
            sortOrder: sortOrder,
            checksum: nil,
    ...
    nonisolated static func safeFileExtension(for draft: PreparedMomentMedia) -> String {
        let sourceExtension = draft.fileURL.pathExtension.lowercased()
        if sourceExtension.range(of: #"^[a-z0-9]{1,10}$"#, options: .regularExpression) != nil {
    ...
    nonisolated static func replaceFile(at sourceURL: URL, withCopyAt destinationURL: URL) throws {
    ...
    func localPlayableURL(for media: CheckInMedia) async throws -> URL {
        if media.hasLocalPlayableFile {
            return URL(fileURLWithPath: media.localCompressedPath)
        }
        ...
        let downloadedURL = try await withMediaDownloadTimeout(seconds: 120) {
            try await self.withAvailableAPIClient(token: token) { client in
                try await client.downloadCheckInMediaFile(mediaId: media.id, variant: "compressed")
            }
        }
        let localURL = try localURLForDownloadedCheckInMedia(media)

        if FileManager.default.fileExists(atPath: localURL.path) {
            try FileManager.default.removeItem(at: localURL)
        }
        try FileManager.default.moveItem(at: downloadedURL, to: localURL)
        try database.markCheckInMediaDownloaded(mediaId: media.id, localPath: localURL.path)
        try await reload()
