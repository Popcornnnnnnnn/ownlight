...
extension LocalDatabase {
    ...
    func fetchTags(type: String? = nil, includeArchived: Bool = false) throws -> [TimelineTag] {
        ...
        if let type {
            sql = """
            SELECT id, type, name, normalizedName, colorHex, isDefault, isArchived, aiUsableAsPrimary, createdAt, updatedAt, archivedAt, areaId
            FROM local_tags
            WHERE type = ?
    ...
    func fetchTagAlias(id: String) throws -> TimelineTagAlias? {
        let statement = try prepare(
            """
            SELECT id, tagId, alias, normalizedAlias, createdAt, deletedAt
            FROM local_tag_aliases
        ...
        try bind(id, to: 1, in: statement)
        if sqlite3_step(statement) == SQLITE_ROW {
            return try timelineTagAlias(statement)
        }
    ...
    func fetchAssignedTags(postId: String) throws -> [TimelineAssignedTag] {
        let statement = try prepare(
            ...
            SELECT pt.id, pt.postId, pt.tagId, pt.role, pt.source, pt.confidence, pt.aiSummaryId,
                   ...
                   t.aiUsableAsPrimary, t.createdAt, t.updatedAt, t.archivedAt, t.areaId
    ...
    func fetchAssignedTags(tagId: String, includeDeleted: Bool = false) throws -> [TimelineAssignedTag] {
        let statement = try prepare(
            """
            SELECT pt.id, pt.postId, pt.tagId, pt.role, pt.source, pt.confidence, pt.aiSummaryId,
                   pt.createdAt, pt.updatedAt, pt.deletedAt,
            ...
            JOIN local_tags t ON t.id = pt.tagId
            WHERE pt.tagId = ?
              AND (? = 1 OR pt.deletedAt IS NULL)
            ORDER BY pt.updatedAt DESC, pt.id ASC
            """
        )
        defer {
            sqlite3_finalize(statement)
    ...
    func fetchAssignedTag(id: String) throws -> TimelineAssignedTag? {
        let statement = try prepare(
            """
            SELECT pt.id, pt.postId, pt.tagId, pt.role, pt.source, pt.confidence, pt.aiSummaryId,
                   pt.createdAt, pt.updatedAt, pt.deletedAt,
        ...
        try bind(id, to: 1, in: statement)
        if sqlite3_step(statement) == SQLITE_ROW {
            return try timelineAssignedTag(statement)
        }
    ...
    func upsertTag(_ tag: TimelineTag) throws {
        let storedAreaId = tag.isTopic
            ? TopicTagArea.fromProviderValue(tag.areaId, topicName: tag.name).rawValue
            : nil
        let statement = try prepare(
            """
            INSERT INTO local_tags
                (id, type, name, normalizedName, colorHex, isDefault, isArchived, aiUsableAsPrimary, createdAt, updatedAt, archivedAt, areaId)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        try bind(storedAreaId, to: 12, in: statement)
    ...
    func upsertAISummary(_ summary: TimelineAISummary) throws {
        ...
        try bind(jsonString(summary.documentBlocks), to: 26, in: statement)
        try stepDone(statement)
    }
    ...
    func timelineTag(_ statement: OpaquePointer, offset: Int = 0) throws -> TimelineTag {
        TimelineTag(
            ...
            areaId: optionalText(statement, Int32(offset + 11))
        )
    }
