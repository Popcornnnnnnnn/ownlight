...
enum EditDraftStore {
    ...
    struct DraftMetadata: Equatable {
        var text: String
        var occurredAt: Date
        var updatedAt: Date
        var existingMediaIds: [String]
        var hasNewMediaDrafts: Bool
    }
    ...
    static func load(postId: String, currentItem: TimelineItem) -> Draft? {
        do {
            ...
            return Draft(
                ...
                updatedAt: file.updatedAt ?? metadataModifiedAt(postId: postId) ?? file.occurredAt,
    ...
    static func loadMetadata(postId: String) -> DraftMetadata? {
        do {
            let data = try Data(contentsOf: metadataURL(postId: postId))
            let file = try decoder.decode(DraftFile.self, from: data)
            return DraftMetadata(
                ...
                occurredAt: file.occurredAt,
                updatedAt: file.updatedAt ?? metadataModifiedAt(postId: postId) ?? file.occurredAt,
                existingMediaIds: file.media.filter { $0.kind == "existing" }.map(\.id),
                hasNewMediaDrafts: file.media.contains { $0.kind != "existing" }
            )
        } catch {
            return nil
        }
    ...
    static func save(
        ...
        let draft = DraftFile(text: text, occurredAt: occurredAt, updatedAt: updatedAt, media: media)
    ...
    static func saveMetadata(
        ...
        let directory = draftDirectory(postId: postId, create: true)
        let media = existingMediaIds.map { DraftMedia(id: $0, kind: "existing", filename: nil) }

        let existingFiles = (try? FileManager.default.contentsOfDirectory(at: directory, includingPropertiesForKeys: nil)) ?? []
        for file in existingFiles where file.lastPathComponent != "draft.json" {
    ...
    private static func metadataModifiedAt(postId: String) -> Date? {
        (try? metadataURL(postId: postId).resourceValues(forKeys: [.contentModificationDateKey]))?.contentModificationDate
    }
