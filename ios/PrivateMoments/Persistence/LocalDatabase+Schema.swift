...
extension LocalDatabase {
    ...
    func migrate() throws {
        try execute(
            ...
            CREATE TABLE IF NOT EXISTS local_checkin_ai_summaries (
                id TEXT PRIMARY KEY,
                entryId TEXT NOT NULL REFERENCES local_checkin_entries(id) ON DELETE CASCADE,
                mediaId TEXT NOT NULL UNIQUE REFERENCES local_checkin_media(id) ON DELETE CASCADE,
                status TEXT NOT NULL,
                format TEXT,
                language TEXT,
                overview TEXT,
            ...
            CREATE INDEX IF NOT EXISTS idx_local_checkin_ai_summaries_entryId ON local_checkin_ai_summaries(entryId);
            CREATE INDEX IF NOT EXISTS idx_local_checkin_ai_summaries_mediaId ON local_checkin_ai_summaries(mediaId);
            CREATE INDEX IF NOT EXISTS idx_local_checkin_ai_summaries_deletedAt ON local_checkin_ai_summaries(deletedAt);

            CREATE TABLE IF NOT EXISTS local_memory_link_events (
                id TEXT PRIMARY KEY,
                postId TEXT NOT NULL,
                shownDate TEXT NOT NULL UNIQUE,
                sourceWindow TEXT NOT NULL,
            ...
            CREATE INDEX IF NOT EXISTS idx_local_memory_link_events_postId ON local_memory_link_events(postId);
            CREATE INDEX IF NOT EXISTS idx_local_memory_link_events_shownAt ON local_memory_link_events(shownAt);

            CREATE TABLE IF NOT EXISTS local_cloudkit_record_states (
                id TEXT PRIMARY KEY,
                entityType TEXT NOT NULL,
                entityId TEXT NOT NULL,
                recordType TEXT NOT NULL,
                recordName TEXT NOT NULL,
                zoneName TEXT NOT NULL DEFAULT 'PrivateMomentsV1',
                recordChangeTag TEXT,
                lastKnownRecordJson TEXT,
                localContentHash TEXT,
                cloudDeletedAt TEXT,
                lastMappedAt TEXT NOT NULL,
                lastUploadedAt TEXT,
                lastDownloadedAt TEXT,
                UNIQUE(entityType, entityId),
                UNIQUE(zoneName, recordName)
            );

            CREATE INDEX IF NOT EXISTS idx_local_cloudkit_record_states_entity
                ON local_cloudkit_record_states(entityType, entityId);
            CREATE INDEX IF NOT EXISTS idx_local_cloudkit_record_states_record
                ON local_cloudkit_record_states(zoneName, recordName);

            CREATE TABLE IF NOT EXISTS local_cloudkit_pending_changes (
                id TEXT PRIMARY KEY,
                entityType TEXT NOT NULL,
                entityId TEXT NOT NULL,
                recordStateId TEXT REFERENCES local_cloudkit_record_states(id) ON DELETE CASCADE,
            ...
            CREATE INDEX IF NOT EXISTS idx_local_cloudkit_pending_changes_status
                ON local_cloudkit_pending_changes(status, nextAttemptAt, createdAt);
            CREATE INDEX IF NOT EXISTS idx_local_cloudkit_pending_changes_entity
                ON local_cloudkit_pending_changes(entityType, entityId);

            CREATE TABLE IF NOT EXISTS local_cloudkit_sync_state (
                scope TEXT PRIMARY KEY,
                serverChangeTokenData BLOB,
                lastAccountStatus TEXT,
        ...
        try addColumnIfNeeded(table: "local_ai_summaries", column: "inputTokenCount", definition: "INTEGER")
        try addColumnIfNeeded(table: "local_ai_summaries", column: "outputTokenCount", definition: "INTEGER")
        try addColumnIfNeeded(table: "local_ai_summaries", column: "totalTokenCount", definition: "INTEGER")
        try addColumnIfNeeded(table: "local_tags", column: "areaId", definition: "TEXT")
        try addColumnIfNeeded(table: "local_checkin_media", column: "durationSeconds", definition: "REAL")
        try addColumnIfNeeded(table: "local_checkin_media", column: "transcriptionText", definition: "TEXT")
        try addColumnIfNeeded(table: "local_checkin_media", column: "transcriptionStatus", definition: "TEXT NOT NULL DEFAULT 'not_requested'")
        try addColumnIfNeeded(table: "local_checkin_media", column: "transcriptionError", definition: "TEXT")
        try addColumnIfNeeded(table: "local_checkin_media", column: "transcriptionUpdatedAt", definition: "TEXT")
        try addColumnIfNeeded(table: "local_checkin_ai_summaries", column: "inputTokenCount", definition: "INTEGER")
        try addColumnIfNeeded(table: "local_checkin_ai_summaries", column: "outputTokenCount", definition: "INTEGER")
        try addColumnIfNeeded(table: "local_checkin_ai_summaries", column: "totalTokenCount", definition: "INTEGER")
        ...
        try addColumnIfNeeded(table: "local_cloudkit_sync_state", column: "lastErrorMessage", definition: "TEXT")
        try execute("CREATE INDEX IF NOT EXISTS idx_local_media_deletedAt ON local_media(deletedAt)")
        try backfillTopicAreas()
        try seedDefaultTags()
    }
    ...
    func backfillTopicAreas() throws {
        var updates: [(id: String, areaId: String)] = []
        ...
        do {
            ...
            while sqlite3_step(statement) == SQLITE_ROW {
                ...
                let name = try text(statement, 1)
                let areaId = optionalText(statement, 2)
                guard !TopicTagArea.isFixedAreaId(areaId) else {
                    continue
                }

                updates.append((
                    id: id,
                    areaId: TopicTagArea.fromProviderValue(areaId, topicName: name).rawValue
                ))
            }
        }
        ...
        try transaction {
            ...
            for update in updates {
                ...
                try bind(update.areaId, to: 1, in: statement)
                try bind(update.id, to: 2, in: statement)
                try stepDone(statement)
            }
        }
    }
