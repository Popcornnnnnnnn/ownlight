...
extension TimelineStore {
    ...
    func runSyncPass(database: LocalDatabase, deviceId: String, token: String) async throws {
        ...
        let audioMediaCountsByPost = pendingMedia.contains(where: \.isAudio)
            ? try database.fetchAudioMediaCountsByPost()
            : [:]
            ...
            do {
                let uploaded = try await withAvailableAPIClient(token: token) { client in
                    try await client.uploadMedia(
                        media,
                        variant: "compressed",
                        audioGroupCount: media.isAudio ? audioMediaCountsByPost[media.postId] : nil
                    )
                }
        ...
        for media in pendingCheckInMedia {
            do {
                ...
                if media.isAudio && uploaded.variant == "compressed" {
                    didUploadSummarizableMedia = true
                }
            } catch {
