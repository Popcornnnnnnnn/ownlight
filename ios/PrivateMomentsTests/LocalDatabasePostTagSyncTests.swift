...
final class LocalDatabasePostTagSyncTests: XCTestCase {
    ...
    func testPersistsTopicAreaOnTags() throws {
        let database = try LocalDatabase.openForTesting(url: temporaryRoot.appending(path: "area.sqlite"))
        let now = Date(timeIntervalSince1970: 1_800)
        var tag = topicTag(id: "topic-product", name: "标签筛选", now: now)
        tag.areaId = TopicTagArea.productDesign.rawValue

        try database.upsertTag(tag)

        let fetched = try XCTUnwrap(database.fetchTag(id: tag.id))
        XCTAssertEqual(fetched.resolvedArea, .productDesign)
    }

    func testClassifiesMissingAndLegacyUncategorizedTopicAreas() throws {
        let database = try LocalDatabase.openForTesting(url: temporaryRoot.appending(path: "area-classify.sqlite"))
        let now = Date(timeIntervalSince1970: 1_800)
        var missingArea = topicTag(id: "topic-security", name: "HTTPS 中间人攻击", now: now)
        missingArea.areaId = nil
        var legacyUncategorized = topicTag(id: "topic-filter", name: "标签筛选", now: now)
        legacyUncategorized.areaId = "uncategorized"

        try database.upsertTag(missingArea)
        try database.upsertTag(legacyUncategorized)

        let security = try XCTUnwrap(database.fetchTag(id: missingArea.id))
        let filter = try XCTUnwrap(database.fetchTag(id: legacyUncategorized.id))
        XCTAssertEqual(security.areaId, TopicTagArea.technology.rawValue)
        XCTAssertEqual(filter.areaId, TopicTagArea.productDesign.rawValue)
    }
