...
final class TimelineSearchTests: XCTestCase {
    ...
    func testTopicFilterMatchesAreaAndAnySelectedTopicByDefault() {
        let item = timelineItem(
            text: "",
            media: [],
            comments: [],
            tags: [
                assignedTag(name: "标签筛选", tagId: "topic-filter", area: .productDesign),
                assignedTag(name: "SwiftUI 性能", tagId: "topic-swiftui", area: .technology)
            ]
        )

        XCTAssertTrue(TimelineTagFilter.matches(
            item,
            selectedArea: .productDesign,
            selectedTopicTagIds: [],
            topicMatchMode: .any
        ))
        XCTAssertTrue(TimelineTagFilter.matches(
            item,
            selectedArea: nil,
            selectedTopicTagIds: ["topic-missing", "topic-swiftui"],
            topicMatchMode: .any
        ))
        XCTAssertFalse(TimelineTagFilter.matches(
            item,
            selectedArea: nil,
            selectedTopicTagIds: ["topic-missing", "topic-swiftui"],
            topicMatchMode: .all
    ...
    func testTopicAreaResolutionClassifiesLegacyUncategorizedValues() {
        var tag = TimelineTag(
            id: "topic-filter",
            type: "topic",
            name: "标签筛选",
        ...
        XCTAssertEqual(tag.resolvedArea, .productDesign)

        tag.name = "跑步训练"
        tag.areaId = nil
        XCTAssertEqual(tag.resolvedArea, .healthFitness)
    }
    ...
    private func assignedTag(name: String, tagId: String, area: TopicTagArea = .life) -> TimelineAssignedTag {
        let now = Date(timeIntervalSince1970: 1_800)
        let tag = TimelineTag(
            id: tagId,
            type: "topic",
