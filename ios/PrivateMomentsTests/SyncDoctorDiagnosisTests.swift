...
final class SyncDoctorDiagnosisTests: XCTestCase {
    ...
    func testMacUnavailableRecommendsSyncNowRetry() {
        ...
        XCTAssertEqual(
            diagnosis.recommendedGuidanceKey,
            "Start with one Sync Now retry. If it still fails, verify the configured Server URL and that the Mac server is reachable."
        )
    }
    ...
    func testCurrentRejectedOperationsRequireManualInspectionWhenPendingWorkRemains() {
        ...
        XCTAssertEqual(
            diagnosis.recommendedGuidanceKey,
            "Do not retry blindly. Inspect Sync Health and Mac logs first so you know what the server rejected."
        )
    }
