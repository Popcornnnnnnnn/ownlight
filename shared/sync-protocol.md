...
- `tags`: primary/topic tag entries with stable IDs, canonical names, normalized names, optional primary color, default/archive flags, `aiUsableAsPrimary`, and optional topic `areaId`.
...
- `upsert_tag` uses `entityType: "tag"` and payload `{type, name, colorHex, isDefault, aiUsableAsPrimary, areaId, updatedAt}`. `areaId` is optional and only applies to topic tags; missing or legacy/unknown values are classified into one of the fixed topic Areas from the topic name before storage.
...
- Timeline no longer surfaces primary tags in normal UI. Topic tags are grouped by fixed Areas in Tags and Timeline Filter.
- Timeline does not show the `synced` success badge; only abnormal sync states remain visible.
- Search can match tag names and aliases. Filters can narrow by Area and then by topic tags; topic multi-select uses OR by default with an optional match-all mode.
- Settings > Tags manages topic Areas, cleanup suggestions, aliases, archive/restore, and queues sync operations like other local-first edits. Legacy primary rows remain stored for compatibility but live under Advanced/Legacy UI only.
