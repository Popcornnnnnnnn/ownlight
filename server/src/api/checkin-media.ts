...
import { markCheckInSummaryDeletedForMedia } from "../ai/checkin-summary-service.js";
...
export async function registerCheckInMediaRoutes(
  ...
  app.post<{ Body: { mediaIds?: unknown; variant?: unknown } }>(
    ...
    async (request, reply) => {
      ...
      if (variant !== "compressed") {
        return sendBadRequest(reply, "check-in media currently supports compressed downloads only");
      }
  ...
  app.post("/api/v1/checkin-media/upload", async (request, reply) => {
    ...
    if (!isAllowedCheckInMediaUpload(file.mimetype, fields.kind)) {
      file.file.resume();
      return sendBadRequest(reply, "Check-in media MIME type does not match the declared kind");
    }
    ...
    try {
      ...
      if (media.kind !== "audio" || media.deletedAt) {
        await markCheckInSummaryDeletedForMedia(context.prisma, media.id);
      }
