...
async function applyDeleteCheckInItem(
  ...
  const itemMediaIds = await tx.checkInMedia.findMany({
    where: {
      entryId: {
        in: itemEntryIds.map((entry) => entry.id),
      },
  ...
  for (const media of itemMediaIds) {
    await markCheckInSummaryDeletedForMedia(tx, media.id, deletedAt);
  }
...
async function applyDeleteCheckInEntry(
  ...
  const entryMediaIds = await tx.checkInMedia.findMany({
    where: {
      entryId: entry.id,
    },
    select: {
  ...
  for (const media of entryMediaIds) {
    await markCheckInSummaryDeletedForMedia(tx, media.id, deletedAt);
  }
