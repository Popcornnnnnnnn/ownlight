...
import { adminEnabledForPlatform } from "../auth/admin-authorization.js";
...
export async function registerAuthRoutes(
  ...
  app.post("/api/v1/auth/login", async (request, reply) => {
    ...
    const retryAfterSeconds = loginRateLimiter.retryAfterSeconds(rateLimitKey);
    if (retryAfterSeconds !== null) {
      await context.fileLogger.warn("auth.login_rate_limited", {
        deviceName: body.deviceName,
        platform: body.platform,
        retryAfterSeconds,
      });
      return sendTooManyRequests(reply, "Too many failed login attempts", retryAfterSeconds);
    }
    ...
    const device = await upsertLoginDevice(context.prisma, {
      ...
      deviceName: body.deviceName,
      platform: body.platform,
      adminEnabled: adminEnabledForPlatform(body.platform),
