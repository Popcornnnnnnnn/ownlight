...
export function parseMediaUploadFields(fields: MultipartFields): MediaUploadFieldsResult {
  ...
  const audioGroupCount = getMultipartInteger(fields, "audioGroupCount");
  ...
  if (!MEDIA_KINDS.has(kind)) {
    return { ok: false, message: "kind must be one of: image, video, audio, document" };
  }
  ...
  if (audioGroupCount !== null && (audioGroupCount < 1 || audioGroupCount > 9)) {
    return { ok: false, message: "audioGroupCount must be between 1 and 9" };
  }
