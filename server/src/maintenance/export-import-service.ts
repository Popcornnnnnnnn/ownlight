...
import { serializeCheckInAISummary } from "../ai/checkin-summary-service.js";
...
export class ExportImportService {
  ...
  async createExport(input: { from?: string; to?: string }): Promise<Record<string, unknown>> {
    ...
    const checkInAiSummaries = checkInEntries.flatMap((entry) => entry.aiSummaries);
    ...
    const manifest: ExportPackageManifest = {
      ...
      counts: {
        ...
        checkInAiSummaries: checkInAiSummaries.length,
...
function serializeCheckInEntryRecord(entry: ExportCheckInItem["entries"][number]): Record<string, unknown> {
  return {
    ...
    aiSummaries: entry.aiSummaries.map(serializeCheckInAISummary),
  };
}
...
async function clearImportedRuntime(tx: Prisma.TransactionClient): Promise<void> {
  ...
  await tx.checkInAiSummary.deleteMany();
...
async function importCheckIns(tx: Prisma.TransactionClient, archive: ExportArchiveFile): Promise<void> {
  for (const item of archive.checkInItems) {
    ...
    for (const raw of item.entries) {
      ...
      for (const mediaRecord of media) {
        ...
        await tx.checkInMedia.create({
          data: {
            ...
            compressedPath: safeImportedMediaPath(getNullableString(mediaRecord, "compressedPath")),
            mimeType: getNullableString(mediaRecord, "mimeType"),
            durationSeconds: getNullableNumber(mediaRecord, "durationSeconds"),
      ...
      const rawSummaries = raw["aiSummaries"];
      const summaries = Array.isArray(rawSummaries) ? rawSummaries.filter(isRecord) : [];
      for (const summaryRecord of summaries) {
        const summaryId = getString(summaryRecord, "id");
        const mediaId = getString(summaryRecord, "mediaId");
        if (!summaryId || !mediaId) {
          continue;
        }

        await tx.checkInAiSummary.create({
          data: {
            id: summaryId,
            entryId: id,
            mediaId,
            status: getString(summaryRecord, "status") ?? "ready",
            format: getNullableString(summaryRecord, "format"),
            language: getNullableString(summaryRecord, "language"),
            overview: getNullableString(summaryRecord, "overview"),
            keyPointsJson: JSON.stringify(arrayValue(summaryRecord["keyPoints"])),
            sectionsJson: JSON.stringify(arrayValue(summaryRecord["sections"])),
            summaryText: getNullableString(summaryRecord, "summaryText"),
            documentTitle: getNullableString(summaryRecord, "documentTitle"),
            oneLiner: getNullableString(summaryRecord, "oneLiner"),
            documentBlocksJson: JSON.stringify(arrayValue(summaryRecord["documentBlocks"])),
            inputTranscriptHash: getNullableString(summaryRecord, "inputTranscriptHash"),
            inputTranscriptLength: getNullableInteger(summaryRecord, "inputTranscriptLength"),
            inputDurationSeconds: getNullableNumber(summaryRecord, "inputDurationSeconds"),
            promptVersion: getString(summaryRecord, "promptVersion") ?? "imported",
            provider: getNullableString(summaryRecord, "provider"),
            model: getNullableString(summaryRecord, "model"),
            errorCode: getNullableString(summaryRecord, "errorCode"),
            errorMessage: getNullableString(summaryRecord, "errorMessage"),
            createdAt: getDate(summaryRecord, "createdAt") ?? new Date(),
            updatedAt: getDate(summaryRecord, "updatedAt") ?? new Date(),
            deletedAt: getNullableDate(summaryRecord, "deletedAt"),
          },
        });
      }
    }
    ...
    for (const raw of post.media) {
      ...
      await tx.media.create({
        data: {
          ...
          compressedPath: safeImportedMediaPath(getNullableString(raw, "compressedPath")),
          originalPath: safeImportedMediaPath(getNullableString(raw, "originalPath")),
          thumbnailPath: safeImportedMediaPath(getNullableString(raw, "thumbnailPath")),
...
async function rebuildServerChanges(tx: Prisma.TransactionClient, archive: ExportArchiveFile): Promise<void> {
  ...
  for (const item of archive.checkInItems) {
    ...
    for (const entry of item.entries) {
      ...
      const rawSummaries = entry["aiSummaries"];
      const summaries = Array.isArray(rawSummaries) ? rawSummaries.filter(isRecord) : [];
      for (const summary of summaries) {
        const summaryId = getString(summary, "id");
        if (!summaryId) {
          continue;
        }
        const summaryChange = await tx.serverChange.create({
          data: {
            entityType: "checkin_ai_summary",
            entityId: summaryId,
            changeType: getNullableString(summary, "deletedAt")
              ? "checkin_ai_summary_deleted"
              : "checkin_ai_summary_updated",
            payloadJson: JSON.stringify(summary),
          },
        });
        checkInItemVersion.set(item.id, summaryChange.version);
      }
    }
  }
...
async function copyImportedMedia(exportDir: string, importedPaths: DataPaths): Promise<void> {
  ...
  await assertSafeImportedMediaTree(source);
  await rm(importedPaths.mediaDir, { recursive: true, force: true });
  await cp(source, importedPaths.mediaDir, { recursive: true });
  await assertSafeImportedMediaTree(importedPaths.mediaDir);
}

function safeImportedMediaPath(relativePath: string | null): string | null {
  if (!relativePath) {
    return null;
  }
  ...
  if (
    ...
    throw new Error("Imported media path must stay inside the media directory");
  }
  ...
  const allowedPrefixes = [
    ...
    "media/originals/",
    "media/thumbnails/",
    "media/checkins/compressed/",
  ];
  if (!allowedPrefixes.some((prefix) => normalized.startsWith(prefix))) {
    throw new Error("Imported media path uses an unsupported media directory");
  }

  return normalized;
}

async function assertSafeImportedMediaTree(directory: string): Promise<void> {
  const entries = await readdir(directory, { withFileTypes: true });
  for (const entry of entries) {
    const child = path.join(directory, entry.name);
    if (entry.isSymbolicLink()) {
      throw new Error("Imported media package must not contain symlinks");
    }
    if (entry.isDirectory()) {
      await assertSafeImportedMediaTree(child);
      continue;
    }
    if (!entry.isFile()) {
      throw new Error("Imported media package must contain only regular files");
    }
  }
}
...
async function verifyImportedData(paths: DataPaths): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    return {
      ...
      checkInAiSummaries: checkInSummaries,
      checkInItems,
      checkInEntries,
      checkInMedia,
      serverChanges,
...
async function countMissingMediaFiles(prisma: PrismaClient, paths: DataPaths): Promise<number> {
  ...
  for (const item of media) {
    ...
    for (const relativePath of [item.compressedPath, item.originalPath, item.thumbnailPath]) {
      ...
      if (!(await safeRegularFileStatsInside(paths.dataDir, absolute))) {
        missing += 1;
      }
    }
  }
