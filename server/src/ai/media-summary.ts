...
export interface MediaSummaryGenerationResult {
...
export interface MediaTranscriptSegmentInput {
  label: string;
  transcriptText: string;
}
...
export function combineAudioTranscriptSegments(segments: MediaTranscriptSegmentInput[]): string {
  const combined = segments
    .map((segment) => ({
      label: segment.label.trim(),
      transcriptText: segment.transcriptText.trim(),
    }))
    .filter((segment) => segment.label.length > 0 && segment.transcriptText.length > 0)
...
export async function generateMediaSummary(
  ...
  return (await generateMediaSummaryWithSource(config, input, usageContext)).summary;
}

export async function generateMediaSummaryWithSource(
  config: AISummaryConfig,
  input: MediaSummaryInput,
  usageContext?: AIUsageContext,
): Promise<MediaSummaryGenerationResult> {
  if (!hasAnySummaryApiKey(config)) {
  ...
  let output = ensureDocumentTitleForRecognizableSummary(
    normalizeSummaryTagsForContext(validateSummaryOutput(response.value), input),
  );
...
export async function generateMediaSummaryFromFile(
  ...
  return {
    ...
    ...(await generateMediaSummaryWithSource(config, {
...
export async function generateMediaSummaryFromFiles(
  config: AISummaryConfig,
  inputs: MediaFileSummaryInput[],
  hooks: MediaSummaryGenerationHooks = {},
): Promise<MediaFileSummaryOutput> {
  if (inputs.length === 0) {
    throw new AISummaryProviderError("media_file_missing", "Media file is not available on the Mac server");
  }

  if (inputs.length === 1) {
    return generateMediaSummaryFromFile(config, inputs[0], hooks);
  }

  const segments: MediaTranscriptSegmentInput[] = [];
  for (const [index, input] of inputs.entries()) {
    try {
      const transcriptText = await transcribeMediaFile(config, input);
      segments.push({
        label: `Recording ${index + 1}`,
        transcriptText,
      });
    } catch (error) {
      if (error instanceof AISummaryProviderError && error.code === "empty_transcript") {
        continue;
      }
  ...
  const transcriptText = combineAudioTranscriptSegments(segments);
  await hooks.onTranscriptReady?.();

  return {
    transcriptHash: transcriptHash(transcriptText),
    transcriptLength: transcriptText.length,
    ...(await generateMediaSummaryWithSource(config, {
      transcriptText,
      durationSeconds: sumDurationSeconds(inputs),
      aiLanguage: inputs[0].aiLanguage ?? "auto",
      existingTopicTags: inputs[0].existingTopicTags,
    }, hooks.usageContext)),
  };
}

function sumDurationSeconds(inputs: MediaFileSummaryInput[]): number | null {
  let total = 0;
...
async function generateMediaSummaryFromAudioInput(
  ...
  const response = await callChatCompletionsWithAudio(
    config,
    input,
    hooks.usageContext,
  );
  const summary = ensureDocumentTitleForRecognizableSummary(
    normalizeSummaryTagsForContext(validateSummaryOutput(response.value), {
      durationSeconds: input.durationSeconds,
    }),
  );
...
async function callChatCompletions(
  ...
  return executeWithConfiguredAIProvider(config, { inputChars }, async (provider) => {
    if (!provider.apiKey) {
      throw new AISummaryProviderError("not_configured", "AI summary provider is not configured");
    }
    ...
    try {
      const response = await fetch(`${provider.baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          response_format: responseFormatForProvider(provider, "media_summary", summaryJsonSchema()),
        }),
      });
...
function hasAnySummaryApiKey(config: AISummaryConfig): boolean {
...
async function callTagCompletions(
  ...
  const systemContent = [
    ...
    "Do not return primary tags.",
    "Choose exactly one fixed area when the transcript has meaningful content: 技术, 产品与设计, 学习与知识, 工作事务, 生活记录, 健康与运动, 情绪与关系.",
    ...
    "Return confidence numbers from 0 to 1. For recognizable content, topic confidence should normally be at least 0.5.",
  ].join("\n");
  const userContent = [
  ...
  const responseResult = await executeWithConfiguredAIProvider(config, { inputChars }, async (provider) => {
    if (!provider.apiKey) {
      throw new AISummaryProviderError("not_configured", "AI summary provider is not configured");
    }
    ...
    try {
      const response = await fetch(`${provider.baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          response_format: responseFormatForProvider(provider, "media_tags", tagSuggestionJsonSchema()),
        }),
      });
      ...
      try {
        ...
        await recordCompletionUsage(usageContext, {
          ...
          response: parsed,
          durationMs: Date.now() - startedAt,
          errorCode: "tag_invalid_json",
        });
        throw new AISummaryProviderError("tag_invalid_json", "AI provider returned invalid tag JSON");
      }
    } catch (error) {
      if (error instanceof AISummaryProviderError) {
        throw error;
...
function systemPrompt(aiLanguage: AILanguagePreference = "auto"): string {
  return [
    ...
    "Do not choose or return primary tags. Choose one fixed area for the note, then concrete topic tags inside that area.",
    "Fixed areas: 技术, 产品与设计, 学习与知识, 工作事务, 生活记录, 健康与运动, 情绪与关系.",
    ...
    "Returning { area: null, topics: [] } for a recognizable spoken note is a failure. For a short app/product feature note, use area 产品与设计 and topic tags like 标签系统 or 颜色设置 when present.",
...
function userPrompt(input: MediaSummaryInput): string {
  ...
  return [
    ...
    "- Suggested tags: do not return primary. Choose one fixed area and use topics for concrete reusable subjects, not broad expression types. Tag suggestions are still required even when the summary is oneLiner-only. Do not return area null and empty topics unless the transcript is content-free.",
...
function audioUserPrompt(input: MediaFileSummaryInput): string {
  return [
    ...
    "Suggested tags: do not return primary. Choose one fixed area and use topics for concrete reusable subjects, not broad expression types. Tag suggestions are still required even when the summary is oneLiner-only. Do not return area null and empty topics unless the audio is content-free.",
    "Topic tags should be sparse. Prefer one topic for one subject; use multiple only for genuinely separate themes.",
    ...
    properties: {
      ...
      suggestedTags: {
        ...
        properties: {
          area: {
            type: "string",
            enum: ["技术", "产品与设计", "学习与知识", "工作事务", "生活记录", "健康与运动", "情绪与关系"],
          },
          topics: {
            type: "array",
            maxItems: 3,
...
function parseSuggestedTags(value: unknown): MediaSummaryTagOutput {
  ...
  const area = parseTopicArea(value.area ?? value.areaId);
  const topics = parseTagSuggestionArray(value.topics, 3).map((topic) => ({
    ...topic,
...
function isEmptySuggestedTags(value: MediaSummaryTagOutput): boolean {
  return value.topics.length === 0;
}

function needsTagFallback(value: MediaSummaryTagOutput): boolean {
  return isEmptySuggestedTags(value);
}
...
function mergeSuggestedTags(
  ...
  return {
    ...
    area: current.area ?? fallback.area,
    topics: current.topics.length > 0 ? current.topics : fallback.topics,
  };
}

function parseTopicArea(value: unknown): string | null {
  const area = parseNonEmptyString(value, 40);
  if (!area) {
    return null;
  }
...
function parseTagSuggestion(value: unknown): MediaSummaryTagSuggestion | null {
  ...
  return {
    ...
    area: parseTopicArea(value.area ?? value.areaId),
  };
}
