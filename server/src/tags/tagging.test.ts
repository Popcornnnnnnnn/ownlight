...
import {
  findReusableTopicTagForName,
  resolveTopicAreaId,
  type TopicTagReuseCandidate,
} from "./tagging.js";
...
test("topic area resolution never returns uncategorized", () => {
  assert.equal(resolveTopicAreaId(null, "HTTPS 中间人攻击"), "technology");
  assert.equal(resolveTopicAreaId("未分类", "标签筛选"), "product_design");
  assert.equal(resolveTopicAreaId("unknown", "跑步训练"), "health_fitness");
  assert.equal(resolveTopicAreaId(null, ""), "life");
});
