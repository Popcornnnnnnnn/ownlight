...
import {
  ...
  normalizeReviewBodyMarkdown,
  REVIEW_PROMPT_VERSION,
  reviewInputDigest,
  synthesizeLegacyReviewBodyMarkdown,
...
const REVIEW_PRIMARY_TAG_ID = "tag-primary-review";
const REVIEW_MEMORY_SCOPE = "periodic_review";
const REVIEW_MEMORY_FEEDBACK_KEY = "feedback_preferences";
const REVIEW_FEEDBACK_CUSTOM_GUIDANCE = "custom_guidance";
const REVIEW_FEEDBACK_STANDARD_TYPES = [
  "useful",
  "too_much_inference",
  "too_dry",
  "missed_point",
...
export class ReviewService {
  ...
  async generate(input: GenerateReviewInput): Promise<Review> {
    const started = await this.startGeneration(input);
    ...
    const current = await this.getReview(started.id);
    return current ?? started;
  }

  async startGeneration(input: GenerateReviewInput): Promise<Review> {
    ...
    const task = this.generateUnlocked(created, input);
  ...
  private async generateUnlocked(created: Review, input: GenerateReviewInput): Promise<Review> {
    try {
      const inputPack = await this.buildInputPack(input);
      const digest = reviewInputDigest(inputPack);
      const generation = await generateReviewWithSource(this.context.config.aiSummary, inputPack, {
  ...
  async startRegeneration(reviewId: string): Promise<Review | null> {
    const existing = await this.getReview(reviewId);
    if (!existing) {
      return null;
    }
  ...
  async setFeedback(reviewId: string, input: {
    type: string;
    enabled: boolean;
    note?: string | null;
  }): Promise<void> {
    const note = normalizeFeedbackNote(input.note);
    await this.context.prisma.reviewFeedback.deleteMany({
      where: {
        reviewId,
        type: input.type,
      },
      ...
      await this.context.prisma.reviewFeedback.create({
        data: {
          ...
          metadataJson: JSON.stringify({
            priority: input.type === REVIEW_FEEDBACK_CUSTOM_GUIDANCE ? "very_high" : "standard",
          }),
        },
      });
    }

    await this.rebuildFeedbackMemory();
  }
  ...
  async publishAsMoment(reviewId: string): Promise<{ review: Review; postId: string } | null> {
    ...
    const updated = await this.context.prisma.$transaction(async (tx) => {
      ...
      const reviewTag = await tx.tag.findUnique({
        where: {
          id: REVIEW_PRIMARY_TAG_ID,
        },
      });
      if (reviewTag && reviewTag.type === "primary" && !reviewTag.isArchived) {
  ...
  private async buildInputPack(input: GenerateReviewInput): Promise<ReviewInputPack> {
    ...
    return {
      ...
      feedbackPreferences: extractFeedbackPreferences(memories),
      highPriorityGuidance: extractHighPriorityGuidance(memories),
  ...
  private async rebuildFeedbackMemory(): Promise<void> {
    const feedbackRows = await this.context.prisma.reviewFeedback.findMany({
      orderBy: {
        createdAt: "desc",
      },
    });
    ...
    const activeTypesOrdered: string[] = [];
    const recentFeedback = feedbackRows.slice(0, 10).map((feedback) => ({
      type: feedback.type,
      note: feedback.note,
    ...
    for (const feedback of feedbackRows) {
      if (!isStandardFeedbackType(feedback.type)) {
        continue;
      }

      standardCounts[feedback.type] = (standardCounts[feedback.type] ?? 0) + 1;
      if (!activeTypesOrdered.includes(feedback.type)) {
        activeTypesOrdered.push(feedback.type);
      }
    }

    const customGuidance = feedbackRows.find((feedback) =>
      feedback.type === REVIEW_FEEDBACK_CUSTOM_GUIDANCE && typeof feedback.note === "string" && feedback.note.trim().length > 0
    );

    const nextValue = {
      activeTypes: activeTypesOrdered,
      activeTypeCounts: standardCounts,
      highPriorityGuidance: customGuidance
        ? {
            note: customGuidance.note,
            reviewId: customGuidance.reviewId,
            at: customGuidance.createdAt.toISOString(),
            weight: "very_high",
          }
        : null,
      recentFeedback,
      lastFeedback: recentFeedback[0] ?? null,
    };
...
function extractFeedbackPreferences(
  memories: Array<{ key: string; valueJson: string }>,
): ReviewInputPack["feedbackPreferences"] {
  const memory = memories.find((item) => item.key === REVIEW_MEMORY_FEEDBACK_KEY);
  const value = parseJsonObject(memory?.valueJson);
  const activeTypes = Array.isArray(value.activeTypes)
    ? value.activeTypes.filter((item): item is string => typeof item === "string" && item.length > 0)
    : [];
...
function extractHighPriorityGuidance(
  memories: Array<{ key: string; valueJson: string }>,
): ReviewInputPack["highPriorityGuidance"] {
  const memory = memories.find((item) => item.key === REVIEW_MEMORY_FEEDBACK_KEY);
  const value = parseJsonObject(memory?.valueJson);
  const guidance = isRecord(value.highPriorityGuidance) ? value.highPriorityGuidance : null;
  return guidance && typeof guidance.note === "string" && guidance.note.trim().length > 0
    ? guidance.note.trim()
    : undefined;
}
...
function parseReviewContent(value: string): ReviewOutput {
  ...
  return {
    ...
    bodyMarkdown: typeof parsed.bodyMarkdown === "string"
      ? normalizeReviewBodyMarkdown(parsed.bodyMarkdown)
      : synthesizeLegacyReviewBodyMarkdown(parsed, typeof parsed.language === "string" ? parsed.language : null),
...
function normalizeFeedbackNote(note: string | null | undefined): string | null {
...
function isStandardFeedbackType(value: string): value is typeof REVIEW_FEEDBACK_STANDARD_TYPES[number] {
  return REVIEW_FEEDBACK_STANDARD_TYPES.includes(value as typeof REVIEW_FEEDBACK_STANDARD_TYPES[number]);
}
