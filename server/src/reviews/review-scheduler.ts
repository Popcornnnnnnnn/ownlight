...
export class ReviewScheduler {
  ...
  private async tick(now = new Date()): Promise<void> {
    ...
    const publishResult = settings.publishWeeklyToMoments && review.status === "ready"
      ? await this.reviews.publishAsMoment(review.id)
      : null;
    ...
    await this.fileLogger.info("review.auto_weekly_triggered", {
      ...
      publishedPostId: publishResult?.postId ?? review.publishedPostId ?? null,
    });
  }
