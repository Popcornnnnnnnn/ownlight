...
DEVICE_LAUNCH_ARGS=()
DEVICE_LAUNCH_ARG_COUNT=0
if [[ -n "${PRIVATE_MOMENTS_DEVICE_LAUNCH_ARGS:-}" ]]; then
  # shellcheck disable=SC2206
  DEVICE_LAUNCH_ARGS=(${PRIVATE_MOMENTS_DEVICE_LAUNCH_ARGS})
  DEVICE_LAUNCH_ARG_COUNT=${#DEVICE_LAUNCH_ARGS[@]}
fi
...
if [[ $DEVICE_LAUNCH_ARG_COUNT -gt 0 ]]; then
  echo "Launch args: ${DEVICE_LAUNCH_ARGS[*]}"
fi
if [[ $DEVICE_LAUNCH_ARG_COUNT -gt 0 ]]; then
  launch_command=(xcrun devicectl device process launch --device "$DEVICE_NAME" "$BUNDLE_ID" "${DEVICE_LAUNCH_ARGS[@]}")
else
  launch_command=(xcrun devicectl device process launch --device "$DEVICE_NAME" "$BUNDLE_ID")
fi
if ! "${launch_command[@]}" 2>&1 | tee "$launch_log"; then
