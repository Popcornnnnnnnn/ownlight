...
function checkTrackedSecrets() {
  ...
  const personalResult = commandOutput("sh", [
    "-lc",
    `git grep -n -E ${shellQuote("100\\.[0-9]{1,3}\\.[0-9]{1,3}\\.[0-9]{1,3}|(^|[^A-Za-z0-9_-])moments\\.popcornnn\\.xyz|wwz 的 iphone")} -- ':!docs/OPEN-SOURCE-READINESS.md' ':!docs/OPERATOR-RUNBOOK.md' ':!AGENTS.md' ':!scripts/open-source-scan.mjs'`,
  ], { timeoutMs: 15_000 });
...
function checkIgnoredRuntimeFiles() {
  ...
  for (const runtimePath of runtimePaths) {
    // Public snapshots intentionally omit generated runtime paths. --no-index
    // still evaluates the ignore rules when the path does not exist.
    const result = commandOutput("git", ["check-ignore", "--no-index", "-q", runtimePath], { timeoutMs: 10_000 });
    if (!result.ok) {
...
function checkPlanningArchivePolicy() {
  const archivePath = path.join(rootDir, ".planning", "_legacy-gsd");
  const manifestPath = path.join(rootDir, ".planning", "LEGACY-GSD-ARCHIVE.md");
  if (!existsSync(manifestPath)) {
    reporter.warn(".planning legacy archive", "LEGACY-GSD-ARCHIVE.md missing; document the old planning archive cleanup boundary");
    return;
  }

  if (existsSync(archivePath)) {
    reporter.warn(".planning legacy archive", ".planning/_legacy-gsd still exists in the checkout; remove it before public-source snapshots");
  } else {
    reporter.pass(".planning legacy archive", "legacy GSD tree is removed from active checkout and cleanup boundary is documented");
  }
}
