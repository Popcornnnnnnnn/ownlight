...
当前结论：**Ownlight 首个公开源码 checkpoint 已发布。公开仓库为 `https://github.com/Popcornnnnnnnn/ownlight`，source-only Release 为 `v1.1.0`；现有私有开发仓库 `Popcornnnnnnnn/private-moments` 继续保持 private，不公开旧提交、backup branches 或内部 release history。**
...
`npm run doctor:release` 负责扫描当前 tracked checkout。公开仓库以已审计当前树的单一初始快照为根，因此不携带私有开发仓库历史；其后只增加公开环境兼容修复。后续只需要继续扫描 public repository 自己的新提交和 release tag。
...
4. 为 public repository 创建 annotated source tag 和 GitHub Release；不要上传 owner-signed IPA。
